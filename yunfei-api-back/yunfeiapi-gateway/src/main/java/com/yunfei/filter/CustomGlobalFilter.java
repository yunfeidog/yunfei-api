...
public class CustomGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> handleResponse(ServerWebExchange exchange, GatewayFilterChain chain, Long interfaceId, Long userId) {
                ...
                public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                    ...
                    if (body instanceof Flux) {
                        ...
                        return super.writeWith(fluxBody.collectList().flatMap(list -> {
                            ...
                            // 读取响应体的内容并转换为字节数组
                            // 得到响应体
                            responseContentLength = (long) content.length;
                            ...
                            return Mono.just(bufferFactory.wrap(content)).doFinally(signalType -> logSave());
                        }));
                    } else {
        ...
        return chain.filter(exchange).doFinally(signalType -> logSave());
    }
